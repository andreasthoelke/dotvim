...
func! Tdb_query_textObj( sel_str )
  " echoe a:sel_str
  " return
  call Tdb_runQueryShow( a:sel_str )
endfunc


func! Tdb_queryAllObjectFields_InnerFields( select_clause )
  ...
  let objFields = Tdb_getObjectFieldsWTL( obj_name )
  let objFields = Tdb_addStrFieldsToObjFields( objFields )
  ...
  call Tdb_runQueryShow( [query] )
  " Can't parse/turn this string into a dictionary
  " let resLines = Tdb_runQuery( [query] )
...
endfunc
" call Tdb_queryAllObjectFieldsTable( 'select Region' )
...
func! Tdb_queryAllObjectFieldsTablePermMulti( obj_name )
  ...
  let fieldNames = Tdb_getObjectFields( a:obj_name )
...
" call Tdb_queryAllObjectFieldsTable( 'Vampire' )
...
func! Tdb_prependModule( obj_name )
...
func! Tdb_queryAllObjectFields( obj_name )
  let obj_name = split( a:obj_name, '\.' )[0]
  let query = 'select count( ' . Tdb_prependModule(obj_name) . ' );'
  let query = query . 'select ' . Tdb_prependModule(obj_name) . ' {*};'
  call Tdb_runQueryShow( [query] )
endfunc

func! Tdb_queryAllObjectFields_withInnerObjs( obj_name )
  let obj_name = split( a:obj_name, '\.' )[0]
  let query = 'select count( ' . Tdb_prependModule(obj_name) . ' );'
  let query = query . 'select ' . Tdb_prependModule(obj_name) . ' {**};'
  call Tdb_runQueryShow( [query] )
...
func! Tdb_queryDeleteObject( obj_name )
  let obj_name = split( a:obj_name, '\.' )[0]
  let query = 'delete ' . Tdb_prependModule(obj_name) . ";"
  call Tdb_runQueryShow( [query] )
endfunc



func! Tdb_showObjectFields( obj_name )
  let fieldNames = Tdb_getObjectFields( a:obj_name )
...
func! Tdb_getObjectFields( obj_name )
...
func! Tdb_showObjectFieldsWT( obj_name )
  let fieldNames = Tdb_getObjectFieldsWT( a:obj_name )
  let g:floatWin_win = FloatingSmallNew ( fieldNames, 'cursor' )

  silent call Tdb_bufferMaps()
  " set syntax=edgeql
  call easy_align#easyAlign( 1, line('$'), ',')
  silent exec "%s/,//ge"
...
func! Tdb_getObjectFieldsWT( obj_name )
  let q1 = "with infos := (select schema::ObjectType { links: { name }, properties: { name } } filter .name = 'default::"
...
func! Tdb_getObjectFieldsWTP( obj_name )
  let q1 = "with infos := (select schema::ObjectType { links: { name }, properties: { name } } filter .name = 'default::"
  let q2 = "'), links_cl := (select infos.links filter .name != '__type__'), properties_cl := (select infos.properties filter .name != 'id'), fields := (properties_cl union links_cl), select fields.name ++ ',' ++ fields.target.name"
  let query = q1 . a:obj_name . q2
  return Tdb_runQuery( [query] )
endfunc
...
func! Tdb_getObjectFieldsWTL( obj_name )
  let list_names_types = Tdb_getObjectFieldsWT( a:obj_name )
  ...
  return res
endfunc
" echoe Tdb_getObjectFieldsWTL( 'Region' )
...
" obj_fields: [{'name': 'other_places', 'isLinkType': 1, 'type': 'OtherPlace', 'isArray': 0}, {'name': 'castles', 'isLinkType': 1, 'type': 'Castle', 'isArray': 0}, {'name': 'cities', 'isLinkType': 1, 'type': 'City', 'isArray': 0}, {'name': 'important_places', 'isLinkType': 0, 'type': 'strA', 'isArray': 1}, {'name': 'name', 'isLinkType': 0, 'type': 'str', 'isArray': 0}, {'name': 'modern_name', 'isLinkType': 0, 'type': 'str', 'isArray': 0}, {'name': 'coffins', 'isLinkType': 0, 'type': 'int16', 'isArray': 0}]
func! Tdb_addStrFieldsToObjFields( obj_fields )
  let res = []
  for field in a:obj_fields
    if field.isLinkType
      let list_names_types = Tdb_getObjectFieldsWTL( field.type )
  ...
  return res
endfunc
" echo Tdb_addStrFieldsToObjFields([{'name': 'other_places', 'isLinkType': 1, 'type': 'OtherPlace', 'isArray': 0}, {'name': 'castles', 'isLinkType': 1, 'type': 'Castle', 'isArray': 0}, {'name': 'cities', 'isLinkType': 1, 'type': 'City', 'isArray': 0}, {'name': 'important_places', 'isLinkType': 0, 'type': 'strA', 'isArray': 1}, {'name': 'name', 'isLinkType': 0, 'type': 'str', 'isArray': 0}, {'name': 'modern_name', 'isLinkType': 0, 'type': 'str', 'isArray': 0}, {'name': 'coffins', 'isLinkType': 0, 'type': 'int16', 'isArray': 0}])


func! Tdb_query_withProp( text )
...
func! Tdb_query_inParans( details )
...
endfunc
" call Tdb_query_inParans( 'ein' )
" see search flags like bnWn /opt/homebrew/Cellar/neovim/0.6.0/share/nvim/runtime/doc/eval.txt#/search.{pattern}%20[,%20{flags}

func! Tdb_query_objCount( obj )
  let line = 'select count( ' . Tdb_prependModule(a:obj) . ' );'

  call Tdb_runQueryShow( [line] )
endfunc
...
func! Tdb_filterProp( lines, propStr )
...
" Tdb_filterProp( resLines, "link __type__" )
...
func! Tdb_describe_object( obj_name )
  let obj_name = a:obj_name
  let obj_name = split( obj_name, "(" )[0] " for function names
  let cmd = "gel describe object " . Tdb_prependModule(obj_name) 
...
command! TdbStartInstance call Tdb_startInstance()

func! Tdb_startInstance ()
...
command! TdbShowTypes call Tdb_showTypes()
...
command! TdbShowSchema call Tdb_describe_schema()

command! TdbShowAllObjects call Tdb_showAllObjects()

func! Tdb_showAllObjects()
  call Tdb_runQueryShow( ['select count( Object ); select Object { __type__: {name} }'] )
endfunc
